...
export async function initAccounts() {
    ...
    try {
            ...
            await userRepository
                .create({
                    ...
                    password: await argon2.hash(
                        process.env.GENERAL_ACCOUNT_PASSWORD!
                    ),
                    name: "Zenith",
                    birthDate: {
                        date: "2025-01-24",
        ...
        if (!personalAccount && companyAccount) {
            ...
            const newPersonalAccount = await userRepository
                .create({
                    username: "vincent",
                    email: process.env.PERSONAL_EMAIL,
                    password: await argon2.hash(
                        process.env.PERSONAL_ACCOUNT_PASSWORD!
                    ),
        ...
        if (!supportAccount && companyAccount) {
            ...
            const newSupportAccount = await userRepository
                .create({
                    username: "support",
                    email: process.env.SUPPORT_EMAIL,
                    password: await argon2.hash(
